...
export class RequestService {
    ...
    static async findOrCreate(
      ...
      if (typeof request === "undefined") {
        if (
          ...
          throw new Error(
            "Request creation requires all data"
          );
        }
       ...
       let requestMetadata = await RequestStatusMetadata.get(request_commitment);
        ...
        if (typeof requestMetadata === "undefined") {
          requestMetadata = RequestStatusMetadata.create({
            ...
            sourceBlockTransaction: sourceBlockTransaction ? sourceBlockTransaction : "",
            messageRelayedTransactionHash: messageRelayedTransactionHash ? messageRelayedTransactionHash : "",
            destFinalizedTransactionHash: destFinalizedTransactionHash ? destFinalizedTransactionHash : "",
            deliveryTransactionHash: deliveryTransactionHash ? deliveryTransactionHash : "",
          });
    ...
    */
    static async updateRequestStatus(
      ...
      let request = await Request.get(request_commitment);
      let requestMetadata = await RequestService.findOrCreateMetadata(request_commitment);

      if (request) {
        request.status = status;
        sourceBlockNumber ? requestMetadata.sourceBlockNumber = sourceBlockNumber : "";
        sourceBlockTransaction ? requestMetadata.sourceBlockTransaction = sourceBlockTransaction: "";
        messageRelayedTransactionHash ? requestMetadata.messageRelayedTransactionHash = messageRelayedTransactionHash : "";
        destFinalizedTransactionHash ? requestMetadata.destFinalizedTransactionHash = destFinalizedTransactionHash : "";
        deliveryTransactionHash ? requestMetadata.deliveryTransactionHash = deliveryTransactionHash : "";
    ...
    static computeRequestCommitment(
      ...
      let hash = solidityKeccak256(["bytes", "bytes", "uint64", "uint64", "bytes", "bytes", "bytes"], [source, dest, nonce, timeoutTimestamp, from, to, body]);
      return hash;
    }
}
